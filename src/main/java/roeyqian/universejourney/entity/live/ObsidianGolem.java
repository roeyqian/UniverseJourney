...
public class ObsidianGolem extends AbstractGolem implements NeutralMob {
    ...
    public ObsidianGolem(final EntityType<? extends ObsidianGolem> type, final Level level) {
        super(type, level);
    }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, Mob.class, 5, false, false, (target, _) -> target instanceof Enemy && !(target instanceof Creeper)));
    ...
    public boolean checkSpawnObstruction(final LevelReader level) {
        ...
        if (!below.entityCanStandOn(level, belowPos, this)) {
            ...
            for(int i = 1; i < 3; ++i) {
                ...
                if (!NaturalSpawner.isValidEmptySpawnBlock(level, abovePos, above, above.getFluidState(), this.getType())) {
                    return false;
                }
            }

            return NaturalSpawner.isValidEmptySpawnBlock(level, pos, level.getBlockState(pos), Fluids.EMPTY.defaultFluidState(), this.getType()) && level.isUnobstructed(this);
        }
    }
    ...
    static {
        DATA_FLAGS_ID = SynchedEntityData.defineId(ObsidianGolem.class, EntityDataSerializers.BYTE);
